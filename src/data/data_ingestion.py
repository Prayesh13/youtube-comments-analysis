...
def concatenate_dataframes(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
    """Concatenate two dataframes and handle potential issues."""
    try:
        # Check if the dataframes have the same columns
        if df1.columns.tolist() != df2.columns.tolist():
            logger.error('Dataframes have different columns: %s vs %s', df1.columns.tolist(), df2.columns.tolist())
            raise ValueError('Dataframes have different columns.')
        
        # Concatenate the dataframes
        combined_df = pd.concat([df1, df2], ignore_index=True)
        logger.debug('Dataframes concatenated successfully.')
        return combined_df
    except Exception as e:
        logger.error('Unexpected error during concatenation: %s', e)
        raise
...
def main():
    try:
        ...
        twitter_df = load_data(data_url='https://raw.githubusercontent.com/PratishMashankar/twitter-sentiment-analysis/refs/heads/master/data/Twitter_Data.csv')
        reddit_df = load_data(data_url='https://raw.githubusercontent.com/Himanshu-1703/reddit-sentiment-analysis/refs/heads/main/data/reddit.csv')

        # Concatenate the dataframes
        df = concatenate_dataframes(twitter_df, reddit_df)
